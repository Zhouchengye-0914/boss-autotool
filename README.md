...
控制台地址为 `http://127.0.0.1:8765`。界面按“概览、搜索与匹配、沟通与 AI、数据与恢复”分区；概览只保留一键开始、继续上次任务和立即停止三个主要入口。搜索、沟通和聊天监听可作为独立模块同时运行，各自保存恢复状态；完整流程仍是独占模式。运行期间保存的新配置只供之后启动的子进程使用。
...
沟通阶段支持 1–4 个独立进程并行。每个实例使用独立 Chrome 用户目录、调试端口、任务分片、断点、CSV、截图和 SQLite 数据库；实例内部仍保持串行随机节奏。首次使用多实例时，在“沟通与 AI”中点击“初始化实例登录”，分别完成一次手动登录。并行沟通实例不会自动发送简历，所有回复监控和附件投递由唯一监控实例执行，避免重复发送。
