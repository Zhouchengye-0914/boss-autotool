...
沟通阶段支持 1–4 个独立进程并行。每个实例使用独立 Chrome 用户目录、调试端口、任务分片、断点、CSV、截图和 SQLite 数据库；实例内部仍保持串行随机节奏。首次使用多实例时，在“高级操作”中点击“初始化全部并行实例登录”，分别完成一次手动登录。并行沟通实例不会自动发送简历，所有回复监控和附件投递由唯一监控实例执行，避免重复发送。
